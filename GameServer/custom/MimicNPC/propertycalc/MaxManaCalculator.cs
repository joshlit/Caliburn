...
namespace DOL.GS.Scripts
{
    ...
    public class MaxManaCalculator : PropertyCalculator
    {
        ...
        public override int CalcValue(GameLiving living, eProperty property)
        {
            ...
            else if (living is MimicNPC && living.Level > 1)
            {
                MimicNPC mimic = living as MimicNPC;
                eStat manaStat = mimic.CharacterClass.ManaStat;
                ...
                if (mimic.CharacterClass.ManaStat == eStat.UNDEFINED)
                {
					 ...
					 * This means that strength ONLY affects a Vampiir's mana pool
					 */
                    if (mimic.CharacterClass.ID == (int)eCharacterClass.Vampiir)
                    {
                        manaStat = eStat.STR;
                    }
                    //else if (mimic.Champion && mimic.ChampionLevel > 0)
                    //{
                    //	return mimic.CalculateMaxMana(mimic.Level, 0);
                    //}
                    else
                    {
                        return 0;
                ...
                int manaBase = mimic.CalculateMaxMana(mimic.Level, mimic.GetModified((eProperty)manaStat));
                int itemBonus = living.ItemBonus[(int)property];
                int poolBonus = living.ItemBonus[(int)eProperty.PowerPool];
                int abilityBonus = living.AbilityBonus[(int)property];

                int itemCap = mimic.Level / 2 + 1;
                int poolCap = mimic.Level / 2;
                itemCap = itemCap + Math.Min(mimic.ItemBonus[(int)eProperty.PowerPoolCapBonus], itemCap);
                poolCap = poolCap + Math.Min(mimic.ItemBonus[(int)eProperty.PowerPoolCapBonus], mimic.Level);
