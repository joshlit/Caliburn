	...
	public class Group : IGameStaticItemOwner
    {
		...
		public IGameStaticItemOwner.TryPickUpResult TryPickUpMoney(IGamePlayer source, GameMoney money)
		{
			...
			foreach (GamePlayer member in GetPlayersInTheGroup())
			{
				...
				if (member.ObjectState is GameObject.eObjectState.Active && member.CanSeeObject(money))
		...
		public IGameStaticItemOwner.TryPickUpResult TryPickUpItem(IGamePlayer source, WorldInventoryItem item)
		{
			...
			foreach (GamePlayer member in GetPlayersInTheGroup())
			{
				if (member.ObjectState is GameObject.eObjectState.Active && member.AutoSplitLoot && member.CanSeeObject(item))
					eligibleMembers.Add(member);
			}
