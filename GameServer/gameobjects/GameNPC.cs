...
namespace DOL.GS
{
    ...
    public class GameNPC : GameLiving, ITranslatableObject
    {
        ...
        {
            get { return m_charStat[eStat.CON - eStat._First]; }
        ...
        public virtual void SayTo(GamePlayer target, eChatLoc loc, string message, bool announce = true)
        {
            ...
            switch (loc)
            {
                case eChatLoc.CL_PopupWindow:
                    ...
                    if (announce)
                    {
                        Message.ChatToArea(this, LanguageMgr.GetTranslation(target.Client.Account.Language, "GameNPC.SayTo.SpeaksTo", GetName(0, true, target.Client.Account.Language, this), target.GetName(0, false)), eChatType.CT_System,
                            WorldMgr.SAY_DISTANCE, target);
                    }
        ...
        public virtual void SortStyles()
        {
            ...
            {
                list = m_styles as ArrayList;
                if (list == null) return;
            }

            foreach (Style s in list.ToArray())
            {
                if (s == null)
                {
